...
data class LoginResponse(val success: Boolean, val token: String?, val user: LoginUser?)
data class LoginUser(val id: String, val email: String, val name: String, val role: String)
data class MeResponse(val user: MeUser?)
data class MeUser(val id: String, val email: String, val name: String, val role: String)
...
class NexusApiClient(private var serverUrl: String) {
        ...
        suspendCancellableCoroutine { cont ->
            client.newCall(request).enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    cont.resumeWithException(IOException("Cannot reach server: ${e.message}"))
                }
                    ...
                    response.use {
                        ...
                        when (it.code) {
                            200 -> {
                                ...
                                if (result.token != null) authToken = result.token
                                cont.resume(result)
                            }
                            401 -> cont.resumeWithException(IOException("Wrong email or password"))
                            else -> cont.resumeWithException(IOException("Login failed (${it.code})"))
                        }
                    }
                }
            })
    ...
    suspend fun getMe(): MeUser? = withContext(Dispatchers.IO) {
        if (authToken == null) return@withContext null
        try {
            val request = Request.Builder().url("$serverUrl/api/auth/me").get().build()
            val response = client.newCall(request).execute()
            if (!response.isSuccessful) { response.close(); return@withContext null }
            val body = response.body?.string() ?: return@withContext null
            response.close()
            val parsed = gson.fromJson(body, MeResponse::class.java)
            parsed.user
