...
class MainActivity : AppCompatActivity() {
    ...
    private val qrScanLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == Activity.RESULT_OK) {
            ...
            if (!url.isNullOrEmpty()) {
                ...
                pendingPairingToken = pairingToken
                connectToServer()
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        apiClient = NexusApiClient(prefs.getString("server_url", "") ?: "")
        ...
        // Detect offline state on startup
        if (!NexusApiClient.isNetworkAvailable(this)) {
            isNetworkAvailable = false
            log("Starting in offline mode — no network detected")
    ...
    private fun buildUI() {
        ...
        val titleText = TextView(this).apply {
            text = "QpiAI Nexus v4"; textSize = 22f; setTextColor(0xFFF0F0F5.toInt())
        ...
        val subtitleText = TextView(this).apply {
            text = "VLM Chat + Vision Detection & Segmentation"
        ...
        // ── SERVER CONNECTION ──
        val serverCard = createCard()
        val serverContent = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL; setPadding(dp(18), dp(18), dp(18), dp(18))
        }
        serverContent.addView(createSectionHeader("\uD83D\uDD17", "Connect to Nexus"))
        serverContent.addView(TextView(this).apply {
            text = "Enter your Cloudflare tunnel URL to sync models"
            textSize = 11f; setTextColor(0xFF6B6B7E.toInt()); setPadding(0, 0, 0, dp(10))
        })
        serverUrlInput = EditText(this).apply {
            ...
            background = ContextCompat.getDrawable(this@MainActivity, R.drawable.input_bg_dark)
            setPadding(dp(16), dp(14), dp(16), dp(14)); isSingleLine = true
        }
        connectButton = Button(this).apply {
            text = "Connect & Register"; textSize = 14f
            setTypeface(null, android.graphics.Typeface.BOLD)
            background = ContextCompat.getDrawable(this@MainActivity, R.drawable.btn_gradient_primary)
        ...
        val scanQrButton = Button(this).apply {
            text = "\uD83D\uDCF7  Scan QR Code"; textSize = 13f; setTextColor(0xFF7B9FC7.toInt())
            background = ContextCompat.getDrawable(this@MainActivity, R.drawable.input_bg_dark)
            setPadding(dp(16), dp(12), dp(16), dp(12)); isAllCaps = false
            ...
            setOnClickListener { qrScanLauncher.launch(Intent(this@MainActivity, QRScanActivity::class.java)) }
        }
        serverContent.addView(serverUrlInput); serverContent.addView(connectButton); serverContent.addView(scanQrButton)
        serverCard.addView(serverContent); container.addView(serverCard)
        ...
        visionModelsContainer.addView(TextView(this).apply {
            ...
            textSize = 11f; setTextColor(0xFF6B6B7E.toInt()); gravity = android.view.Gravity.CENTER
    ...
    private fun createEmptyState(title: String, subtitle: String): LinearLayout = LinearLayout(this).apply {
        ...
        addView(TextView(this@MainActivity).apply { text = "\uD83E\uDD16"; textSize = 28f; gravity = android.view.Gravity.CENTER; setPadding(0, 0, 0, dp(8)) })
        addView(TextView(this@MainActivity).apply { text = title; textSize = 14f; setTextColor(0xFF8B8B9E.toInt()); gravity = android.view.Gravity.CENTER; setTypeface(null, android.graphics.Typeface.BOLD) })
        addView(TextView(this@MainActivity).apply { text = subtitle; textSize = 11f; setTextColor(0xFF6B6B7E.toInt()); gravity = android.view.Gravity.CENTER; setPadding(0, dp(4), 0, 0) })
    }
    ...
    private fun connectToServer() {
        ...
        if (url.isEmpty()) { log("Error: Enter a server URL"); return }
        val imm = getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
        imm.hideSoftInputFromWindow(serverUrlInput.windowToken, 0)
        prefs.edit().putString("server_url", url).apply()
        apiClient.updateServerUrl(url)
        connectButton.isEnabled = false; connectButton.text = "Connecting..."
        log("Connecting to $url ...")

        scope.launch {
            try {
                val pairingToken = pendingPairingToken; pendingPairingToken = null
                val savedDeviceToken = prefs.getString("device_token", null)
                if (pairingToken != null) { log("Using QR pairing token..."); apiClient.setAuthToken(pairingToken) }
                else if (savedDeviceToken != null) { log("Using saved device token..."); apiClient.setAuthToken(savedDeviceToken) }
                else { log("No auth token available") }

                val hw = getDeviceHardware()
                val savedDeviceId = prefs.getString("device_id", null)
                log("Registering device: ${hw.name}")
                val result = apiClient.registerDevice(hw, savedDeviceId)
                deviceId = result.id
                prefs.edit().putString("device_id", result.id).apply()
                if (result.token != null) {
                    prefs.edit().putString("device_token", result.token).apply()
                    apiClient.setAuthToken(result.token)
                    log("Device token stored")
                }
                downloadManager.authToken = apiClient.getAuthToken()
                log("Registered! Device ID: ${result.id}"); log(result.message)
                ...
                connectButton.text = if (isConnected) "Reconnect" else "Connect & Register"
            }
        }
    }
    ...
    private fun createModelRow(model: ModelInfo): View {
        ...
        val subtitleRow = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL; gravity = android.view.Gravity.CENTER_VERTICAL; setPadding(0, dp(3), 0, 0)
        }
        subtitleRow.addView(TextView(this).apply {
    ...
    private fun displayVisionModels(models: List<VisionModelInfo>) {
        ...
        if (models.isEmpty()) {
            visionModelsContainer.addView(TextView(this).apply {
                ...
                gravity = android.view.Gravity.CENTER; setPadding(0, dp(12), 0, dp(12))
            })
            return
        }
        for (model in models) {
    ...
    private fun getCpuUsage(): Int {
        try {
            val line: String; java.io.RandomAccessFile("/proc/stat", "r").use { reader -> line = reader.readLine() }
            val parts = line.split("\\s+".toRegex()); val idle = parts[4].toLong()
            val total = parts.drop(1).take(7).sumOf { it.toLong() }; Thread.sleep(100)
            val line2: String; java.io.RandomAccessFile("/proc/stat", "r").use { reader -> line2 = reader.readLine() }
    ...
    private fun setConnectionStatus(connected: Boolean) {
        ...
        if (connected) {
            ...
            connectButton.text = "Connect & Register"
        } else {
                ...
                runOnUiThread {
                    ...
                    if (inferenceMode == "server") {
                        ...
                        if (localModels.isNotEmpty()) {
                            ...
                            Toast.makeText(this@MainActivity, "Offline — no local models available", Toast.LENGTH_LONG).show()
                        }
                    }
                }
            }
    ...
    private fun updateConnectionStatusForNetwork() {
        ...
        // If connected, setConnectionStatus(true) already handles it
    }
